...
void BraveProfileWriter::UpdateSettings(const SessionStoreSettings& settings) {
  ...
  if (url_service) {
    ...
    if (it != importable_engines.end()) {
      TemplateURLPrepopulateData::PrepopulatedEngine engine = it->second;
      std::unique_ptr<TemplateURLData> template_data =
  ...
  if (prefs) {
    ...
    if (settings.use_alternate_private_search_engine_tor) {
      // if enabled, set to either Qwant or DuckDuckGo (based on region)
      int private_search_engine_id = brave::IsRegionForQwant(profile_)
        ? TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_QWANT
        : TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_DUCKDUCKGO;
      prefs->SetInteger(kAlternativeSearchEngineProviderInTor,
           private_search_engine_id);
    } else {
